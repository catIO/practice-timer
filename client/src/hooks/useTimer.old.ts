...
export function useTimer({ initialSettings, onComplete }: UseTimerProps) {
  ...
  useEffect(() => {
    const detectIOS = () => {
      ...
      if (isIOS) {
        ...
        // Initialize iOS wake lock (userActivity disabled for battery optimization)
        iosWakeLockRef.current = getIOSWakeLock({
          preventScreenTimeout: true,
          preventSystemSleep: true,
          audioContext: true,
          userActivity: false, // Disabled to save battery
          fullscreen: false
        });
      }
    };
  ...
  const completeSession = useCallback(() => {
    if (mode === 'work') {
      ...
      if (currentIteration === totalIterations) {
        console.log('useTimer: Last work session complete, practice is finished!');
        setIsRunning(false);
        setStoreIsRunning(false);
  ...
  }, [mode, settings, currentIteration, totalIterations, setMode, setTimeRemaining, setTotalTime, setIsRunning, setStoreMode, setStoreTimeRemaining, setStoreTotalTime, setStoreIsRunning, setCurrentIteration, setStoreCurrentIteration, updateBackgroundTimer, setIsPracticeComplete]);
  ...
  useEffect(() => {
    return () => {
      ...
      // Release all wake locks on unmount to save battery
    ...
    if (workerRef.current) {
      ...
      messageHandler = (event: MessageEvent) => {
        ...
        if (type === 'TICK') {
          ...
          console.log('useTimer: Received PRACTICE_COMPLETE message from worker');
          console.log('useTimer: PRACTICE_COMPLETE payload:', payload);
          setIsRunning(false);
          setStoreIsRunning(false);
          setIsPracticeComplete(true);
          
          // Show notification immediately when practice completes
          showNotification(
            'Practice Complete!',
            {
              body: `You've completed all ${payload.totalIterations} work sessions!`,
              requireInteraction: true,
              silent: false
            }
          );
          ...
          // Call onComplete callback if provided
          if (onComplete) {
            console.log('useTimer: Calling onComplete callback for practice completion');
            try {
              onComplete();
  ...
  const resetTimer = useCallback(() => {
    ...
    // Update worker state to reset iteration and mode
    if (workerRef.current) {
      workerRef.current.postMessage({
        type: 'UPDATE_MODE',
        payload: {
  ...
  // Start new session (reset practice completion)
  ...
  const startTimer = useCallback(async () => {
    try {
      ...
      // Only use ONE wake lock strategy at a time to save battery
      try {
        ...
        // Don't try fallback as it may cause battery drain
      }
  ...
  // Reduced frequency to save battery - check every 2 minutes instead of 30 seconds
  useEffect(() => {
    ...
    // Check wake lock status less frequently to save battery (every 2 minutes)
    let intervalId: number | null = null;
    if (isRunning) {
      intervalId = window.setInterval(ensureWakeLock, 120000); // 2 minutes instead of 30 seconds
    }
